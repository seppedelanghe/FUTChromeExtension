...
window.actions.transferItems = (() => {
    try {
        services.Item.requestTransferItems().observe(this, (t, resp) => {
            ...
            let inactive = resp.data.items.filter(item => item.getAuctionData().isInactive());
            ...
            window.returnResponse('CustomCodeResponse', {'status': 'Got transfer items!', 'action': 'transfers', 'result': data}); 
        });
    } catch (e) {
